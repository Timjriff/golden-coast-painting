...
export function SiteHeader({home=false}:{home?:boolean}){
 ...
 return (<header className={!home || scrolled || menu ? "header is-scrolled" : "header over-hero"} id="top"><Brand/><nav aria-label="Main navigation">{nav.map(([label,href])=><a key={href} href={href} aria-current={!home && href==='/contact' ? 'page' : undefined}>{label}</a>)}</nav><a className="button" href="/contact#quote">Get a Free Estimate <ArrowUpRight size={18}/></a><Button className="menu-button" variant="ghost" aria-label={menu?'Close navigation':'Open navigation'} aria-expanded={menu} aria-controls="mobile-nav" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</Button>{menu&&<nav id="mobile-nav" className="mobile-nav" aria-label="Mobile navigation">{[...nav,['Get a Free Estimate','/contact#quote']].map(([label,href])=><a key={href} href={href} onClick={()=>setMenu(false)}>{label}<ArrowUpRight size={17}/></a>)}</nav>}</header>);
}
